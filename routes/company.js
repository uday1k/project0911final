...
const checkCompanyAuth = (req, res, next) => {
  if (!(req.isAuthenticated())) {
    ...
    if (req.user.account && req.user.account.role === "company") {
      const companyDetails = new Promise((resolve,reject)=>{
        dbo.collection("Companies").findOne({ companyEmail: req.user.emailid.value },function(err,data){
          resolve(data);
        });
      }) 
      companyDetails.then(function(data){
        if (data.status === "accepted") {
          ...
          return next(createError(403, 'your company not approved.you will receive mail after approval'));
        }
      })
...
router.post('/savejob', checkCompanyAuth, async function (req, res) {
  if (!(req.body.jobmongoid)) {
    ...
    insertJobDetails.companyName = res.locals.companyName;
